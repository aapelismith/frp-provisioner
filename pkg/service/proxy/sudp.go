...
func init() {
	RegisterProxyFactory(reflect.TypeOf(&config.SUDPProxyConfig{}), NewSUDPProxy)
}

type SUDPProxy struct {
	*BaseProxy

	cfg *config.SUDPProxyConfig
...
func NewSUDPProxy(baseProxy *BaseProxy, cfg config.ProxyConfigurer) Proxy {
	unwrapped, ok := cfg.(*config.SUDPProxyConfig)
	if !ok {
		return nil
	}
	return &SUDPProxy{
