...
func init() {
	RegisterProxyFactory(reflect.TypeOf(&config.UDPProxyConfig{}), NewUDPProxy)
}
...
type UDPProxy struct {
	*BaseProxy
	...
	// include api.UDPPacket and api.Ping
	sendCh   chan msg.Message
	workConn net.Conn
	closed   bool
}

func NewUDPProxy(baseProxy *BaseProxy, cfg config.ProxyConfigurer) Proxy {
	unwrapped, ok := cfg.(*config.UDPProxyConfig)
	if !ok {
		return nil
	}
	return &UDPProxy{
