...
var proxyFactoryRegistry = map[reflect.Type]func(*BaseProxy, config.ProxyConfigurer) Proxy{}

func RegisterProxyFactory(proxyConfType reflect.Type, factory func(*BaseProxy, config.ProxyConfigurer) Proxy) {
...
func NewProxy(
	ctx context.Context,
	pxyConf config.ProxyConfigurer,
...
type BaseProxy struct {
	...
	baseCfg        *config.ProxyBaseConfig
	clientCfg      *config.ClientCommonConfig
	msgTransporter transport.MessageTransporter
}
...
func (pxy *BaseProxy) HandleTCPWorkConnection(workConn net.Conn, m *msg.StartWorkConn, encKey []byte) {
	...
	if baseCfg.Transport.ProxyProtocolVersion != "" {
		if m.SrcAddr != "" && m.SrcPort != 0 {
			...
			if baseCfg.Transport.ProxyProtocolVersion == "config" {
				h.Version = 1
