...
func init() {
	RegisterProxyFactory(reflect.TypeOf(&config.XTCPProxyConfig{}), NewXTCPProxy)
}

type XTCPProxy struct {
	*BaseProxy

	cfg *config.XTCPProxyConfig
}

func NewXTCPProxy(baseProxy *BaseProxy, cfg config.ProxyConfigurer) Proxy {
	unwrapped, ok := cfg.(*config.XTCPProxyConfig)
	if !ok {
		return nil
	}
	return &XTCPProxy{
...
func (pxy *XTCPProxy) listenByKCP(listenConn *net.UDPConn, raddr *net.UDPAddr, startWorkConnMsg *msg.StartWorkConn) {
	xl := pxy.xl
	_ = listenConn.Close()
	localAddr, _ := net.ResolveUDPAddr("udp", listenConn.LocalAddr().String())
	lConn, err := net.DialUDP("udp", localAddr, raddr)
	if err != nil {
